 ...
 */
user.getMe = async (req, res) => {
	
	let userId = await helper.getUUIDByTocken(req);
	
	if ( userId ) {
		
		let result = await _userModelObj.getMeData(userId);
 ...
 * @developer : Ashwani Kumar
 */
user.getOtherUser = async (req, res) => {
	
	let userId = req.body.userId;
	
	if ( userId ) {
		
		let result = await _userModelObj.getOtherUserData(userId);
		console.log('result--------',result)
		if ( result && typeof(result) == 'object' ) {
    ...
    if (userId) {
        ...
        if ( req && req.body && req.body.otherUuid ) {
            ...
            if (otherUserId) {
                ...
                let result = await _userModelObj.businessCardRequest(otherUserId,userId);
                //   console.log('dsdasdadadad',result)
                if (result) {

                    helper.successHandler(res, {
                        payload: {
                            businessRequestType: 'P'
                        }
                    }, 200);
