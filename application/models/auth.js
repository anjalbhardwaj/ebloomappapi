            ...
            if ( error ) {
                ...
                if ( results && Object.keys(results).length > 0 && results.insertId ) {
                    ...
                    if ( userId && userId != '' ) {
                        ...
                        // pool.query( insertQuery1, [userId, insteredDate], async (error, result) => {
...
//  console.log('loginloginloginloginlogin=====================>>>>>11111111',body)
//     let deferred       = q.defer(),
...
//         }
//         console.log('loginloginloginloginlogin=====================>>>>>11111111',body.email,body.password)
...
//         // WHERE ` + addCondition;
//         let stepOne         = `SELECT user.u_id, user.u_uuid, user.u_full_address, user.u_name, user.u_email, user.u_description, user.u_password, user.u_image,  user.u_latitude, user.u_longitude, user.u_phone, user.u_gender, user.u_country, user.u_state, user.u_full_address, user.u_is_online, user.u_is_available, user.u_active_count, user.u_activation_token, user.u_active, countries.name as country_name, states.name as state_name
//         FROM user 
//         LEFT JOIN countries ON countries.id = user.u_country 
//         LEFT JOIN states ON states.id = user.u_state 
//         WHERE`  + addCondition;

//         // let stepFourth      = "SELECT n_id from user_notifications WHERE n_fk_receiver_u_id = ? AND n_isseen != ? AND n_deleted = '0'";
...
//             console.log('getUserStepOne111111111111111111111111111');

//             let loginUserData      = getUserStepOne[0] ;
            

//             // let userNotifications = await commonHelper.getDataOrCount( stepFourth, [ loginUserData.u_id,'1' ] , 'L');

//             // loginUserData.total_count = 0;

//             // if (  userNotifications && userNotifications > 0 ) {
//             //     loginUserData.total_count = userNotifications;
//             // }

//             if ( loginUserData.u_password && passwordHash.verify(body.password, loginUserData.u_password ) ) {
//                 // console.log('22222222222222222222222222222');
//                 console.log('getUserStepOne 22222222222222222222222222222222');
...
//                        let aaa =  authModel.updateLastLoginTime(loginUserData.u_id);
//                         console.log('55555555555555 loginDevice : ', loginDevice, aaa);
//                     if ( /* loginDevice &&  */updateDevice ) {
//                         console.log('55555555555555 loginDevice : ', updateDevice);

//                         if ( loginUserData.u_is_online ) {

//                             console.log('55555555555555 loginDevice : ', loginUserData.u_is_online);

//                             loginUserData.u_is_online = '1';
//                         }
//                         console.log('55555555555555 loginDevice : ', loginUserData);
 ...
 authModel.login = async (body, type = '') => {
       ...
       let deferred       = q.defer(),
        ...
        if ( body && body.email && body.password ) {
            ...
            let stepOne         = `SELECT users.id, users.name, users.email,  users.password 
            FROM users 
            WHERE`  + addCondition;
            ...
            let getUserStepOne  =  await commonHelper.getDataOrCount( stepOne, [ body.email], 'D' ,true );
            console.log("getUserStepOnegetUserStepOnegetUserStepOne : ", getUserStepOne);
            if ( getUserStepOne && getUserStepOne.length > 0 && getUserStepOne[0] &&  getUserStepOne[0].id ) {
        
                let loginUserData      = getUserStepOne[0] ;
                
    
               
                if ( loginUserData.password && passwordHash.verify(body.password, loginUserData.password ) ) {
                    deferred.resolve(loginUserData);
                //     // console.log('getUserStepOne 22222222222222222222222222222222');
...
authModel.resetPassword = async function ( body ) {
    console.log('reeee----ddddd',body);

    let deferred = q.defer();

    pool.query('SELECT users.id, users.name FROM users WHERE users.email = ?', [body.email], async function (error, results, fields) {
        console.log('reeee-sssss---',results);
        ...
        if ( error ) {
            ...
            if ( results && results.length > 0 ) {
                ...
                console.log(randomPwd(10));

                let pwd = randomPwd(10)
                hashedPassword = passwordHash.generate(pwd);

                pool.query('UPDATE users SET password = ? WHERE users.id = ?', [ hashedPassword, results[0].id ], async function ( error, row, fields ) {

                    if ( error ) {
                        console.log('reeee-sssss--werrrr-',error);

                        deferred.reject(error);
                    } else {
                        // console.log('reeee-sssss-33l3ll3l3l3ll3--',results);

                        let emailArray = {
                            to      : body.email,
                            from    : 'osrdnprojects@gmail.com',
                            subject : 'Reset password ',
                            body    : "Hi " + results[0].name + ",` here is your new password as requested to reset your password is:" + pwd,
                        };
                        common.sendEmails(emailArray);
                        ...
                        //     activityObj     = {
                        //         userId          : results[0].id,
                        //         actionUserId    : results[0].id,  
                        //         description     : results[0].name + '  requested to reset password at ' + Date.now() + ' And your new password is : '+ pwd, 
                        //         activityType    : 'SIGNUP', 
                        //         date            : date
                        //     };
 ...
 * @developer : Ashwani Kumar
 */
authModel.updateUser = function ( body ) {
    let deferred = q.defer();

    pool.query( body.sql, body.val, function ( error, results ) {
...
authModel.sendWelcomeEmail = async function( userId ) {
    ...
    let objEmail = {
        ...
        text      : "Welcome to the Ebloomapi, please login to your account with your login credentials."
    }
...
authModel.insertUserData = async function( userId,body ) {
    let deferred     =  q.defer(),
        insteredDate =  await commonHelper.getPstDateTime('timeDate'),
        conObj       =  await constant.getConstant();
    ...
    if ( userId ) {
        ...
        let insertQuery1 = 'INSERT INTO user_address ( ua_fk_u_id, ua_created ) VALUES (?,?)';
            res          = await common.commonSqlQuery(insertQuery1,[userId,insteredDate]);
        ...
        let insertQuery2 = 'INSERT INTO user_price ( up_fk_u_id, up_group, up_individual,up_in_person_price, up_online_price,up_instant_call_price,up_video_answer_price,up_text_answer_price,up_message_price,up_hourly_price,up_created ) VALUES (?,?,?,?,?,?,?,?,?,?,?)';
            resOne       = await common.commonSqlQuery(insertQuery2,[ userId, conObj.EXPERT_MIN_PRICE, conObj.EXPERT_MIN_PRICE,conObj.EXPERT_MIN_PRICE, conObj.EXPERT_MIN_PRICE ,price, price,replyPrice, replyPrice ,conObj.EXPERT_MIN_PRICE,insteredDate ]);

        let insertQuery3 = 'INSERT INTO user_bio ( ub_fk_u_id, ub_created ) VALUES (?,?)';
            resThree     = await common.commonSqlQuery(insertQuery3,[ userId,insteredDate ]);

        let insertQuery4 = 'INSERT INTO user_wallet ( uw_fk_u_id, uw_created ) VALUES (?,?)';
            resFour      = await common.commonSqlQuery(insertQuery4,[ userId,insteredDate ]);
