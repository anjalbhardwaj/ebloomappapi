 ...
 */
 ...
 postModel.userPostCount = async (userId, body) => {
 ...
 */
 postModel.getPostData = async (userId, body) => {
    ...
    if ( userId && body ) {
        ...
        if( body.otherUserId ){
            // console.log('fsdfsdfsdf',body.otherUserId)
            ...
            addCondition        = ` WHERE u_deleted = ? AND  u_deactivated = ? AND p_deleted = ? AND u_enable = ? AND p_status = ? `
            dataArray       = [ '0', '0', '0', '1', '1' ];

        }
        
        let sql  = ` SELECT u_id,u_name, u_image, u_uuid, p_id, p_uuid, p_text, p_title, p_like_count, p_post_type, p_comment_count,p_share_count ,p_created, p_share_id FROM post 
        ...
        if ( result && result.sqlMessage ) { 
            ...
            if ( result && result.length > 0 ) {
                ...
                for ( let  resultData of result ) {
                    ...
                    if( resultData.p_created ){
                        resultData.p_created = await helper.agoTime( resultData.p_created );
                    }
                    ...
                    if( resultData.p_share_id ){
                       let otherPostSql =  ` SELECT u_id,u_name, u_image, u_uuid, p_id, p_uuid, p_text, p_title, p_like_count, p_post_type, p_comment_count,p_share_count ,p_created, p_share_id FROM post 
 ...
 */
 postModel.getCommentData = async (userId, body) => {
    ...
    if ( userId && body ) {
        ...
        let sql             = ` SELECT u_id,u_name, u_image, u_uuid, pc_id, pc_uuid, pc_uuid, pc_comment_like_count, pc_comment_reply_count, pc_comment , pc_created FROM post_comments 
 ...
 */
 postModel.getLikeData = async (userId, body) => {
    ...
    if ( userId && body ) {
        ...
        let sql             = ` SELECT u_id,u_name, u_image, u_uuid, pl_id, pl_created FROM post_like 
