...
emailTemplates = require('../../assets/email-templates')
...
floristModel.getOrderData = async (body) => {

    //  console.log('body=====>>>',body)
    let deferred = q.defer(),
        conObj = await constant.getConstant(),
        obj = {};
    let sql = `select * FROM orders WHERE id IN('${body.orderId}')`
    ...
    if (body) {
        ...
        if (result && result.sqlMessage) {
            ...
            if (result && result.length > 0) {
                // processData();
                _SENDMAIL.sendMails(processData(result), body.mails);
...
function processData(a) {
    ...
    a.forEach(x => {
        table;
        tr += `<tr style=" border-bottom: 1pt solid #5b5b5b;box-sizing:border-box;font-size:12px;font-family:'Open Sans',sans-serif;color:#646a6e;line-height:18px;font-weight: 700;padding:10px 0;">`;
        tr += `<td style="width: 14.5rem;"> ${x.name} </td>` +
            `<td><img style="width: 52px;object-fit: cover;height: 50px;" alt="no image " src="${x?.img}" alt=""></td>` +
            `<td>${x.grand_total}€</td>` +
            `<td>${x?.qntity}</td>`
        tr += '</tr>'

        totPrice += x.grand_total;
    })

    table.innerHTML += tr;
    var newTemp = emailTemplates.replace("$newTable", table.innerHTML).replace('$totalPrice', `${totPrice}€`).replace('$grandtotPrice', `${totPrice}€`);
    return newTemp
}
