...
commonModel.sendEmails = async (emailData) => {
    ...
    let transporter = nodeMailer.createTransport({
        ...
        auth    : {
            ...
            user    :'osrdnprojects@gmail.com',// conObj.SENDINBLUE_USER,
            pass    :'qjkmohogreivkiua',// ,// conObj.SENDINBLUE_PASS
        }
    });
    ...
    let mailOptions = {
        from    : emailData.from,// 'Glimpster <' + emailData.from + '>', // sender address
        to      : emailData.to, // list of receivers
        subject : emailData.subject, // Subject line
        html    : emailData.body // html body
    };
    console.log('asdas wsdxwsss1111111sss',);

    transporter.sendMail(mailOptions, (error, info) => {
        console.log('asdas wsdxwssssss',);
    ...
    console.log('asdas wsdxws3333sssss',);
        ...
        if ( result && typeof result == 'object' && Object.keys(result).length > 0 ) {
            ...
            if ( results && typeof results == 'object' && Object.keys(results).length > 0 ) {
                ...
                if ( body.text ) {
                    ...
                    dataEmail += '<br /><br /><strong>Regards,</strong><br /><strong> The Ebloomapi Team </strong>';
...
commonModel.getRowId = function (uuid, wherecolname, selectcolname, tablename) {
    console.log("sendPaymentNotificationsendPaymentNotificationsendPaymentNotificationsendPaymentNotification------------------------------------")
    let deferred = q.defer(),
 ...
 * Modified By  : 
*/
commonModel.getAll = function (uuid, wherecolname, tablename, selectcolname = '*', whereAnd = '') {
    let deferred = q.defer(),
...
commonModel.getUnseenNotCount = async function (userId, isseen) {
    ...
    let deferred = q.defer();
    sql = "SELECT n_id from user_notifications WHERE n_fk_receiver_u_id = ?  AND n_isseen != ? AND n_deleted = '0'";

    if (userId) {

        pool.query(sql, [userId, isseen], function (error, results, fields) {
 ...
 * @ModifiedBy  :  
 */
commonModel.getCountryList = async function (body) {
    ...
    if ( body ) {
        ...
        if ( body.keywords != null && body.keywords != undefined && body.keywords != '' ) {
            ...
            addCondition += " AND name like '%" + body.keywords + "%'";
        }

        let sql = `SELECT id , name , sortname ,CONCAT('` + conObj.API_URL + conObj.UPLOAD_PATH +`country_images/', image ) AS image ,  phonecode FROM countries  WHERE status = ? ` + addCondition;
